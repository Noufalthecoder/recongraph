...
| **Rounding** | When rounding is unavoidable (e.g., fee splits), use **ROUND_HALF_UP** (banker's rounding). All rounding operations MUST be explicit, never implicit. |
...
| **Fee handling** | The gateway fee from the original payment is **not** refunded to the merchant when a refund is processed. The merchant bears the fee cost. |
...
> **SIMULATION ASSUMPTION:** Fee-on-refund behavior (whether fee is reversed) should be verified against actual Razorpay documentation. Our simulation assumes fees are NOT reversed.
...
| Rule | Detail |
|---|---|
| **UTR** | The Unique Transaction Reference is a bank-generated identifier for the actual NEFT/RTGS/IMPS transfer from the gateway's account to the merchant's bank account. |
...
> **SIMULATION ASSUMPTION:** Default settlement schedule is T+2. Cutoff time is 00:00 IST (midnight). Bank holidays are not simulated in the initial version (all weekdays are treated as business days).
...
settlement.amount =
  ...
  - SUM(refund debits)
  - SUM(transfer debits)
  + SUM(adjustment credits)
  - SUM(adjustment debits)
  - SUM(fees)
  - SUM(tax)
```
